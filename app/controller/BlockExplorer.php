...
class ControllerBlockExplorer extends Controller
{
    ...
    {
        // Prüfe Parameter
        if (isset($_GET['p0']) AND $_GET['p0'] >= 2014 AND isset($_GET['p1']) AND $_GET['p1'] >= 1 AND $_GET['p1'] <= 12 AND isset($_GET['p2']) AND $_GET['p2'] >= 1 AND $_GET['p2'] <= 31 AND checkdate($_GET['p1'],
                $_GET['p2'],
                $_GET['p0']) AND strtotime($_GET['p0'].'-'.$_GET['p1'].'-'.$_GET['p2']) >= strtotime("2014-08-11") AND strtotime($_GET['p0'].'-'.$_GET['p1'].'-'.$_GET['p2']) <= strtotime(date("Y-m-d"))
        ) {
    ...
    function address()
    {
        ...
        if (isset($_GET['p0']) AND $_GET['p0'] > 0) {
            ...
            $addressData = $this->db->query_cache("SELECT *, UNIX_TIMESTAMP(accountFirstseen) AS accountFirstseenTS, UNIX_TIMESTAMP(lastActivity) AS lastActivityTS FROM ".DB_PRE."chain_accounts WHERE account='".$this->db->escapeString($_GET['p0'])."'", 120);
            if (isset($addressData[0])) {
                $this->smarty->assign('addressdata', $addressData[0]);
                $this->smarty->assign('firstseenAge',
                    $this->validate->convertAge(time() - $addressData[0]['accountFirstseenTS']));
                $this->smarty->assign('lastActivityAge',
                    $this->validate->convertAge(time() - $addressData[0]['lastActivityTS']));

                // QR-Code erstellen
                if (!file_exists(DIR_PUBLIC.DS.'media'.DS.'qr'.DS.$addressData[0]['accountRS'].'.png')) {
                    QRcode::png($addressData[0]['accountRS'],
                        DIR_PUBLIC.DS.'media'.DS.'qr'.DS.$addressData[0]['accountRS'].'.png', QR_ECLEVEL_H, 3, 1);
                }

                // Zähle die gefundenen Blöcke
                $countBlockData = $this->db->query_cache("SELECT COUNT(*) AS forgedBlocks, SUM(blockReward) AS totalMined, SUM(totalFeeNQT) AS minedFees FROM ".DB_PRE."chain_blocks WHERE generator='".$addressData[0]['account']."'", 120);
                $this->smarty->assign('forgedBlocks', $countBlockData[0]['forgedBlocks']);
                $this->smarty->assign('totalMinedBurst',
                    $countBlockData[0]['totalMined'] + $countBlockData[0]['minedFees']);
                ...
                $totalTransactions = $this->db->query_cache("SELECT COUNT(*) AS transactions FROM ".DB_PRE."chain_transactions WHERE sender='".$addressData[0]['account']."' OR recipient='".$addressData[0]['account']."'", 120);
                ...
                $transactionData = $this->db->query_cache("SELECT t1.transaction, t1.sender, t1.recipient, t1.amountNQT, t1.feeNQT, t1.transactiondate, t2.accountRS AS senderRS, t3.accountRS AS recipientRS FROM ".DB_PRE."chain_transactions AS t1 LEFT JOIN ".DB_PRE."chain_accounts AS t2 ON t1.sender=t2.account LEFT JOIN ".DB_PRE."chain_accounts AS t3 ON t1.recipient=t3.account WHERE t1.sender='".$addressData[0]['account']."' OR t1.recipient='".$addressData[0]['account']."' ORDER BY t1.transactiondate DESC LIMIT ".$sqlOffset.",50", 120);
                $this->smarty->assign('transactiondata', $transactionData);

                // Ermittle die Summe des empfangenen Guthabens
                $totalReceived = $this->db->query_cache("SELECT SUM(amountNQT) AS totalReceived FROM ".DB_PRE."chain_transactions WHERE recipient='".$addressData[0]['account']."'", 120);
                $this->smarty->assign('totalReceived', $totalReceived[0]['totalReceived']);

                // Ermittle die Summe des gesendeten Guthabens
                $totalSent = $this->db->query_cache("SELECT SUM(amountNQT) AS totalSent FROM ".DB_PRE."chain_transactions WHERE sender='".$addressData[0]['account']."'", 120);
                $this->smarty->assign('totalSent', $totalSent[0]['totalSent']);

                $pools = "SELECT DISTINCT recipient as pool FROM ".DB_PRE."chain_transactions WHERE sender={$addressData[0]['account']} and recipient <> 9223372036854775807 and type=20";
                $poolMinedBalance = $this->db->query_cache("SELECT SUM(amountNQT) AS balance FROM ".DB_PRE."chain_transactions WHERE recipient='".$addressData[0]['account']."' AND sender IN ($pools)", DB_MAX_CACHE_TIME);
                $this->smarty->assign('poolMinedBalance', $poolMinedBalance[0]['balance']);

                // Ermittle den Rang in der Rich List
                $richListRank = $this->db->query_cache("SELECT COUNT(*) AS rank FROM ".DB_PRE."chain_accounts WHERE unconfirmedBalanceNQT>='".$addressData[0]['unconfirmedBalanceNQT']."'", DB_MAX_CACHE_TIME);
                ...
                $blockData = $this->db->query_cache("SELECT height, block, blockReward, totalFeeNQT, UNIX_TIMESTAMP(blockdate) AS blockdateTS FROM ".DB_PRE."chain_blocks WHERE generator='".$addressData[0]['account']."' ORDER BY height DESC LIMIT ".$sqlOffset.",20", 120);
                if (isset($blockData[0])) {
                    $parsedBlockData = '';
                    foreach ($blockData AS $blockdata) {
                        $parsedBlockData[] = array(
    ...
    function addressChartTransactions()
    {
        ...
        if (isset($_GET['p0']) AND $_GET['p0'] > 0) {
            ...
            if (isset($addressData[0])) {
                ...
                $starttime = mktime(0, 0, 0, date("m", $addressData[0]['accountFirstseenTS']),
                    date("d", $addressData[0]['accountFirstseenTS']) - 1,
                    date("Y", $addressData[0]['accountFirstseenTS']));
                ...
                foreach ($transactionData AS $transactiondata) {
                    $parsedTransactionData[date("Y-m-d",
                        mktime(0, 0, 0, $transactiondata['transactionmonth'], $transactiondata['transactionday'],
                            $transactiondata['transactionyear']))] = array(
    ...
    function addressExportForgedBlocks()
    {
        ...
        if (isset($_GET['p0']) AND $_GET['p0'] > 0) {
            ...
            if (isset($addressData[0])) {
                ...
                $objPHPExcel->getProperties()->setCreator("Burstcoin")
                    ->setLastModifiedBy("Burstcoin")
                    ...
                    ->setKeywords("Burstcoin forged blocks ".$addressData[0]['accountRS'])
                    ->setCategory("Export");
                ...
                if (isset($blockData[0])) {
                    ...
                    foreach ($blockData AS $block) {
                        ...
                        $objPHPExcel->getActiveSheet()->getCell('A'.$i)->getHyperlink()->setUrl("http://blocks.fastpool.info/block/".$block['block']);
        ...
        if (isset($_GET['p0']) AND $_GET['p0'] > 0) {
            ...
            if (isset($addressData[0])) {
                ...
                $objPHPExcel->getProperties()->setCreator("Burstcoin")
                    ...
                    ->setKeywords("Burstcoin transactions ".$addressData[0]['accountRS'])
                    ->setCategory("Export");
                ...
                $transactionData = $this->db->query("SELECT t1.transaction, t1.sender, t1.recipient, t1.amountNQT, t1.feeNQT, t1.transactiondate, t1.type, t1.subtype, t1.block, t2.accountRS AS senderRS, t3.accountRS AS recipientRS FROM ".DB_PRE."chain_transactions AS t1 LEFT JOIN ".DB_PRE."chain_accounts AS t2 ON t1.sender=t2.account LEFT JOIN ".DB_PRE."chain_accounts AS t3 ON t1.recipient=t3.account WHERE t1.sender='".$addressData[0]['account']."' OR t1.recipient='".$addressData[0]['account']."' ORDER BY t1.transactiondate DESC",
                    true);
                if ($this->db->numRows > 0) {
                    ...
                    while ($transaction = $transactionData->fetch_array(MYSQL_ASSOC)) {
                        ...
                        $objPHPExcel->getActiveSheet()
                            ...
                            ->setCellValue('G'.$i,
                                $conf['transactiontypes'][$transaction['type']][$transaction['subtype']])
                            ->setCellValue('H'.$i, $transaction['block']);

                        // Formatiere die Zellen
                        $objPHPExcel->getActiveSheet()->getStyle('A'.$i)->getNumberFormat()->setFormatCode('0');
                        $objPHPExcel->getActiveSheet()->getCell('A'.$i)->getHyperlink()->setUrl("http://blocks.fastpool.info/transaction/".$transaction['transaction']);
                        $objPHPExcel->getActiveSheet()->getStyle('A'.$i)->applyFromArray($link_style_array);
                        $objPHPExcel->getActiveSheet()->getCell('C'.$i)->getHyperlink()->setUrl("http://blocks.fastpool.info/address/".$transaction['sender']);
                        $objPHPExcel->getActiveSheet()->getStyle('C'.$i)->applyFromArray($link_style_array);
                        $objPHPExcel->getActiveSheet()->getCell('D'.$i)->getHyperlink()->setUrl("http://blocks.fastpool.info/address/".$transaction['recipient']);
                        ...
                        $objPHPExcel->getActiveSheet()->getCell('H'.$i)->getHyperlink()->setUrl("http://blocks.fastpool.info/block/".$transaction['block']);
    ...
    function transaction()
    {
        ...
        if (isset($_GET['p0']) AND $_GET['p0'] > 0) {
            ...
            if (isset($transactionData[0])) {
                ...
                $this->smarty->assign('confirmations',
                    $this->globalstatsData[0]['blocks'] - 1 - $transactionData[0]['height']);
                ...
                $this->smarty->assign('type',
                    $conf['transactiontypes'][$transactionData[0]['type']][$transactionData[0]['subtype']]);
