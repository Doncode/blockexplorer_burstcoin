...
class ControllerHome extends Controller
{
    ...
    function index()
    {
        ...
        if ($this->router->getRoute() == "/search" && isset($_POST['search'])) {
            $this->smarty->assign('errorMsg',
                $this->bootstrap->alert('Unrecognized search pattern', 'danger', '', 'margin: 15px 0 0 0;'));
            ...
            if (!empty($_POST['search'])) {
                ...
                $addressSearch = $this->db->query("SELECT account FROM " . DB_PRE . "chain_accounts WHERE account='" . $this->db->escapeString($_POST['search']) . "' OR accountRS='" . $this->db->escapeString($_POST['search']) . "' OR name='" . $this->db->escapeString($_POST['search']) . "'  LIMIT 1");
                if (isset($addressSearch[0])) {
                    $this->request->redirect(HTTP_ROOT . 'address/' . $addressSearch[0]['account']);
                }

                // Suche nach Transaction ID
                $transactionSearch = $this->db->query("SELECT transaction FROM " . DB_PRE . "chain_transactions WHERE transaction='" . $this->db->escapeString($_POST['search']) . "' LIMIT 1");
                ...
                $blockSearch = $this->db->query("SELECT block FROM " . DB_PRE . "chain_blocks WHERE (height='" . $this->db->escapeString($_POST['search']) . "' OR block='" . $this->db->escapeString($_POST['search']) . "') AND height<>'0'  LIMIT 1");
        ...
        $blockData = $this->db->query("SELECT * FROM " . DB_PRE . "chain_blocks ORDER BY height DESC LIMIT 30", false,
            DB_MAX_CACHE_TIME);
        $parsedBlockData = '';
...
//                $this->model_stats->totalSupply();
        ...
        $blockData = $this->db->query_cache("SELECT COUNT(*) AS blocks FROM " . DB_PRE . "chain_blocks", false,
            DB_MAX_CACHE_TIME);
        $this->smarty->assign('blocks', $blockData[0]['blocks'] - 1);

        // Ermittle die Anzahl an Transaktionen
//                $this->model_stats->totalTransactions();fromCache
