...
class ModelStats extends Model
{
    ...
    public function totalSupply()
    {
        ...
        $getStats = $this->db->query("SELECT SUM(blockReward) AS totalSupply FROM ".DB_PRE."chain_blocks ");
    ...
    public function totalTransactions()
    {
        ...
        $transactionData = $this->db->query("SELECT COUNT(*) AS transactions, DATE_FORMAT(transactiondate,'%Y') AS transactionyear, DATE_FORMAT(transactiondate,'%m') AS transactionmonth, DATE_FORMAT(transactiondate,'%e') AS transactionday FROM ".DB_PRE."chain_transactions WHERE transactiondate>='".date("Y-m-d",
                $this->last30d)."' GROUP BY DATE(transactiondate) ORDER BY transactiondate ASC");
    ...
    public function totalWallets()
    {
        ...
        $walletData = $this->db->query("SELECT COUNT(*) AS wallets, DATE_FORMAT(accountFirstseen,'%Y') AS walletyear, DATE_FORMAT(accountFirstseen,'%m') AS walletmonth, DATE_FORMAT(accountFirstseen,'%e') AS walletday FROM ".DB_PRE."chain_accounts WHERE accountFirstseen>='".date("Y-m-d",
                $this->last30d)."' GROUP BY DATE(accountFirstseen) ORDER BY accountFirstseen ASC");
    ...
    public function networkSize()
    {
        ...
        $blockData = $this->db->query_cache("SELECT baseTarget, DATE_FORMAT(blockdate,'%Y') AS blockyear, DATE_FORMAT(blockdate,'%m') AS blockmonth, DATE_FORMAT(blockdate,'%e') AS blockday, DATE_FORMAT(blockdate,'%k') AS blockhour, DATE_FORMAT(blockdate,'%i') AS blockmin FROM ".DB_PRE."chain_blocks WHERE blockdate>='".date("Y-m-d",
                $this->last30d)."' ORDER BY height ASC", DB_MAX_CACHE_TIME);
        $i = 0;
        $baseTarget = 0;
        foreach ($blockData AS $blockdata) {
            $i++;
